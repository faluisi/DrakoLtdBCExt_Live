/*
-------------------------------------------------------------------------------------------------------
Business Central - Implementation Requirements

This codeunit is built to hold the logic surrounding FBM's operations related to Fixed Assets. 
-------------------------------------------------------------------------------------------------------
*/
codeunit 50000 FixedAssetDimMgt
{
    ...
    procedure RenameCustOpSite(xCustomerSite: Record "Customer-Site"; CustomerSite: Record "Customer-Site")
    var
        COS: Record "Cust-Op-Site";
    begin
        COS.Reset();
        ...
        if COS.FindFirst() then begin
            if COS."Site Code" <> CustomerSite."Site Code" then
                COS.Rename(CustomerSite."Customer No.", CustomerSite."Customer No.", CustomerSite."Site Code");
        end;
    end;

    procedure RenameSiteDim(xCustSite: Record "Customer-Site"; CustSite: Record "Customer-Site")
    var
        DimValue_Site: Record "Dimension Value";
        FASetup: Record "FA Setup";
        companyinfo: Record "Company Information";
    ...
    begin
        ...
        if companyinfo.Get() then begin
            if companyinfo."Customer Is Operator" then
                RenameCustOpSite(xCustSite, CustSite);
        end;
        // end;
    end;

    procedure ModifySiteDim("xCust-Site": Record "Customer-Site"; "Cust-Site": Record "Customer-Site")
    var
        FASetup: Record "FA Setup";
    ...
    /*procedure InsertDimCombination(ChildDim: Code[20]; ChildCode: Code[20]; ParentDim: Code[20]; ParentCode: Code[20])
    var
    ...
    procedure RenameCustDim(xCust: Record Customer; Cust: Record Customer)
    var
    ...
    procedure RenameOpDim(xCustomer: Record Customer; Customer: Record Customer)
    //procedure to use for scenarios where customer = operator
    var
